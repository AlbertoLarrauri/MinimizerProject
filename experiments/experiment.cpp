...
#include "../minimizers/OFAMinimizers/naive.h"
#include <unistd.h>
#include <ctime>
...
void SBCMin::minimizationExperiment(const SBCMin::DFSM &driver, const SBCMin::DFSM &driven,
...

    {
        ...
        auto minimizeOFA = [&]()->Stats {
            ...
            volatile bool success = minimizer.run(ofa, driven.size()-1);
            DFSM result = success? minimizer.getResult(): driven;
...
void SBCMin::minimizationExperimentNaive(const SBCMin::DFSM &driver, const SBCMin::DFSM &driven,
                                         std::filesystem::path log_dir) {
    namespace fs = std::filesystem;
...

    {
        ...
        for (int i = 0; i < 1; ++i) {
            auto report = benchmark<Stats>(minimizeOFA, 600);
            if (report.err != 0) {
                timeout = true;
                break;
            } else {
...
void SBCMin::minimizationExperimentPro(const SBCMin::DFSM &driver, const SBCMin::DFSM &driven,
                                       std::filesystem::path log_dir) {
}
...
void SBCMin::experimentCampaignNaive(std::filesystem::path dir, std::filesystem::path result_dir) {
    namespace fs = std::filesystem;
    if (!fs::exists(dir)) {
        std::cout << "Experiment directory does not exist";
        return;
    ...
    for (auto &entry: fs::directory_iterator(dir)) {
        ...
        auto result_name = filename + "stats" + ".txt";
        auto result_path = result_dir / result_name;
        minimizationExperimentNaive(A, B, result_path);
    }
}
