...
//       std::cout<<"Incompatibility clauses: \n";
    for (auto pair:compat_matrix.getPairs()) {
        ...
        std::vector<CMSat::Lit> clause;
        clause.reserve(2);
        clause.emplace_back(stateClassVar(state1, current_size - 1), true);
        clause.emplace_back(stateClassVar(state2, current_size - 1), true);
//            std::cout<<clause<<"\n";
        solver->add_clause(clause);
    }
        ...
        for (int i = 0; i < ofa.numberOfInputs(); ++i) {
            for (int state1 = 0; state1 < size; ++state1) {
                ...
                for (int state2:succs) {
                    ...
                    clause1.reserve(3);
                    clause2.reserve(3);
                    clause1.emplace_back(classClassVar(Class, current_size - 1, i), true);
                    clause1.emplace_back(stateClassVar(state1, Class), true);
                    clause1.emplace_back(stateClassVar(state2, current_size - 1), false);
                    solver->add_clause(clause1);
                    clause2.emplace_back(classClassVar(current_size - 1, Class, i), true);
                    clause2.emplace_back(stateClassVar(state1, current_size - 1), true);
                    clause2.emplace_back(stateClassVar(state2, Class), false);
...
void BasicIncremental::computeSolution() {
